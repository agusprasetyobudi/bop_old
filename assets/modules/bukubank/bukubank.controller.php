<?php
class bukubank_controller extends CFControllers
{
	public function cfs()
	{
		parent::Boot();
	...
	{
		//breaad crumb
		$output['breadcrumb'] = array('bukubank'=>'Buku Kas','list'=>'List Data Buku Kas');
		...
		for($i=0;$i<count($output['list']);$i++)
		{
			...
			$output['list'][$i]['debet'] = $this->model()->transaksiDebet($output['list'][$i]['uraian']['id']);
		}
		$output['content'] = 'bukubank/list.html';
	...
	public function add()
	{
		...
		{
			$ss = $this->model()->getlist();
			$get_arr = array();
			for($i=0;$i<count($ss);$i++)
			{
				$get_arr[] = $ss[$i]['id_item_transfer'];
			}
			...
			for($i=0;$i<count($output['firm_list']);$i++)
			{
				...
				for($j=0;$j<count($transfer_list);$j++)
				{
					...
					for($k=0;$k<count($output['firm_list'][$i]['transfer_list'][$j]['items']);$k++)
					{
						...
						{
							if(isset($output['firm_list'][$i]['transfer_list'][$j]['items'][$k]['id']) && in_array($output['firm_list'][$i]['transfer_list'][$j]['items'][$k]['id'],$get_arr))
							{
		...
		$output['content'] = 'bukubank/form.html';
		$this->teil('template')->load('page');
		$this->teil('template')->render($output);
	}
	...
	{
		//breaad crumb
		$output['breadcrumb'] = array('bukubank'=>'Buku Kas','list'=>'Report');
